...
namespace THH.Core.Excel.Npoi
{
    ...
    public class ExportExcelHelper<T> : ExcelBaseResult<T> where T : new()
    {
        ...
        /// <param name="isCompress">是否压缩</param>
        /// <returns></returns>
        public static void ExportListToExcel_MvcResult(IList<T> list, string fileName, IList<string> headerList = null, IList<string> sortList = null, bool isCompress = false, int pagingSize = 65535)
        {
            //无数据情况
            if (list == null || list.Count == 0)
            {
